...
#define CKCU_POS_LSISTOP        4
#define CKCU_MASK_LSISTOP       ((u32)0x1 << CKCU_POS_LSISTOP)

#define CKCU_POS_LSESTOP        5
#define CKCU_MASK_LSESTOP       ((u32)0x1 << CKCU_POS_LSESTOP)
...
#define CKCU_POS_TMSEL          (4)
#define CKCU_MASK_TMSEL         ((u32)0x1 << CKCU_POS_TMSEL)
...
#define IS_GAINMODE(GanMode)    ((GanMode == CKCU_HSE_LOW_GAIN_MODE) || \
                                 (GanMode == CKCU_HSE_MEDIUM_LOW_GAIN_MODE)|| \
                                 (GanMode == CKCU_HSE_MEDIUM_HIGH_GAIN_MODE)|| \
                                 (GanMode == CKCU_HSE_HIGH_GAIN_MODE))

#if (LIBCFG_CKCU_LSI_TRIM)

/* HSICR bit field definition                                                                               */
#define CKCU_POS_LSCLKSEL       (8)
#define CKCU_MASK_LSCLKSEL      ((u32)0x1 << CKCU_POS_LSCLKSEL)

/* LSITCR bit field definition                                                                              */
#define CKCU_POS_PTKEY          (16)
#define CKCU_MASK_PTKEY         ((u32)0xFFFF << CKCU_POS_PTKEY)
#define CKCU_POS_LSITRIMEN      (8)
#define CKCU_MASK_LSITRIMEN     ((u32)0x1 << CKCU_POS_LSITRIMEN)
#define CKCU_POS_LSITRIM        (0)
#define CKCU_MASK_LSITRIM       ((u32)0x7F << CKCU_POS_LSITRIM)

#define CKCU_LSITCR_KEY         (0x9B830000)

#define CKCU_HSIATCR_COUNT_PRE_REFCLK_BY_LSI                  (HSI_VALUE / 1000)
...
void CKCU_DeInit(void)
{
  ...
  #if (LIBCFG_CKCU_LSI_TRIM)
  HT_CKCU->LSITCR  = CKCU_LSITCR_KEY;            /* Disable the LSITCR register write protection            */
  HT_CKCU->LSITCR  = 0;                          /* LSITRIMEN = 0 will reset the LSITRIM as default value,  */
                                                 /* this line also set the PTKEY = 0 to enable the write    */
                                                 /* protection function                                     */
  #endif
...
void CKCU_HSIAutoTrimClkConfig(CKCU_ATC_TypeDef CLKSRC)
{
  ...
  if (CLKSRC == CKCU_ATC_LSE)
  {
   CKCU_BF_WRITE(HT_CKCU->HSICR, CKCU_MASK_LSCLKSEL, CKCU_POS_LSCLKSEL, 0);
  }
}
...
void CKCU_HSIAutoTrimCmd(ControlStatus Cmd)
{
  ...
  if (Cmd == ENABLE)
  {
    CKCU_BF_WRITE(HT_CKCU->HSICR, CKCU_MASK_TMSEL, CKCU_POS_TMSEL, DISABLE);
  }
...
#if (LIBCFG_CKCU_LSI_TRIM)
/*********************************************************************************************************//**
 * @brief Configure the LSI Manual Trim By HSI function.
 * @retval None
 * @note  In LSI auto-trim mode, the ATC is set to manual, using the LSI as the reference clock and the HSI
          as the source. The ATC counts HSI pulses to represent the LSI period.
 ************************************************************************************************************/
void CKCU_LSIManualTrimInit(void)
{
  u32 uHSICRTemp;
  u32 uLSITCRTemp;

  /* Disable write protection. It write the PTKEY with the original field, prevent the unexpected field     */
  /* clear if the write protection is disabled before this function.                                        */
  uLSITCRTemp = HT_CKCU->LSITCR & (~(CKCU_MASK_PTKEY));  /* Read the LSITCR value except PTKEY              */
  HT_CKCU->LSITCR = CKCU_LSITCR_KEY | uLSITCRTemp;

  /* Enable LSI Trimming and write protection                                                               */
  HT_CKCU->LSITCR = CKCU_MASK_LSITRIMEN | uLSITCRTemp;

  /* Set the HSICR                                                                                          */
  /* LSCLKSEL  = 0x1 (Low Speed Clock Selection = LSI)                                                      */
  /* REFCLKSEL = 0x0 (Reference Clock Selection = low speed clock source)                                   */
  /* TMSEL     = 0x1 (Manual by user program)                                                               */
  uHSICRTemp = HT_CKCU->HSICR & (~(CKCU_MASK_REFCLKSEL));
  HT_CKCU->HSICR = uHSICRTemp | CKCU_MASK_LSCLKSEL | CKCU_MASK_TMSEL;
}

/*********************************************************************************************************//**
 * @brief Enable or Disable the LSI Manual Trim By HSI function.
 * @param Cmd: This parameter can be ENABLE or DISABLE.
 * @retval None
 * @note  In LSI auto-trim mode, the ATC is set to manual, using the LSI as the reference clock and the HSI
          as the source. The ATC counts HSI pulses to represent the LSI period.
 ************************************************************************************************************/
void CKCU_LSIManualTrimCmd(ControlStatus Cmd)
{
  /* Check the parameters                                                                                   */
  Assert_Param(IS_CONTROL_STATUS(Cmd));
...
/*********************************************************************************************************//**
 * @brief Do the LSI Manual Trim by HSI function and check it is ready or not.
 * @param ErrorCount: The target error count (accuracy = ErrorCount/8000).
 *   This parameter can be set manual or be one of the following define:
 *     @arg   CKCU_LSITRIM_ACCURACY_0P0 : Margin of error +-0.0%
 *     @arg   CKCU_LSITRIM_ACCURACY_0P5 : Margin of error +-0.5%
 *     @arg   CKCU_LSITRIM_ACCURACY_1P0 : Margin of error +-1.0%
 *     @arg   CKCU_LSITRIM_ACCURACY_1P5 : Margin of error +-1.5%
 *     @arg   CKCU_LSITRIM_ACCURACY_2P0 : Margin of error +-2.0%
 * @retval TRUE or FALSE
 * @note  The HSIATCR is updated every 2 ms. It is recommended to use a timer to call the
 *        "CKCU_LSIManualTrimIsReady()" >= 2 ms.
 ************************************************************************************************************/
bool CKCU_LSIManualTrimIsReady(u32 ErrorCount)
{
  u32 atCntrMax;
  u32 atCntrMin;
  bool isReady = TRUE;

  /* Get HSI Auto trimming counter.                                                                         */
  u32 cntr = HT_CKCU->HSIATCR;

  /* Get LSI manual trimming counter.                                                                       */
  u32 LSICurrentTrimValue =  HT_CKCU->LSITCR & CKCU_MASK_LSITRIM;
  ...
  /* Calculate the range of margin of error for LSI trimming counter.                                       */
  atCntrMax = CKCU_HSIATCR_COUNT_PRE_REFCLK_BY_LSI + ErrorCount;
  atCntrMin = CKCU_HSIATCR_COUNT_PRE_REFCLK_BY_LSI - ErrorCount;

  /* Modify LSI trim value                                                                                  */
  if (cntr < atCntrMin)
  {
    LSICurrentTrimValue++;   /* trimming counter + 1 = slower freq                                          */
    isReady = FALSE;
  }
  else if (cntr > atCntrMax)
  {
    LSICurrentTrimValue--;   /* trimming counter - 1 = faster freq                                          */
    isReady = FALSE;
  }

  if ((isReady == FALSE) && (LSICurrentTrimValue <= CKCU_MASK_LSITRIM))
  {
    /* Disable write protection                                                                             */
    HT_CKCU->LSITCR = CKCU_LSITCR_KEY | (HT_CKCU->LSITCR & (~(CKCU_MASK_PTKEY)));

    /* Set the LSI new trimming value                                                                       */
    HT_CKCU->LSITCR = CKCU_MASK_LSITRIMEN | LSICurrentTrimValue;
  }
 ...
 *        @arg CKCU_HSE_MEDIUM_LOW_GAIN_MODE  : HSE medium low gain mode
 *        @arg CKCU_HSE_MEDIUM_HIGH_GAIN_MODE : HSE medium high gain mode
...
void CKCU_SetHSEGainMode(u32 GanMode)
{
  ...
  HT_CKCU->GCCR = (HT_CKCU->GCCR & ~(0x1100)) | GanMode;
}

#if (LIBCFG_CKCU_LSI_LSE_STOP)
/*********************************************************************************************************//**
 * @brief Enable or Disable the LSI stop function in the core power domain.
 * @param Cmd: This parameter can be ENABLE or DISABLE.
 * @retval None
 ************************************************************************************************************/
void CKCU_LSIStopCmd(ControlStatus Cmd)
{
  /* Check the parameters                                                                                   */
  Assert_Param(IS_CONTROL_STATUS(Cmd));

  CKCU_BF_WRITE(HT_CKCU->GCCR, CKCU_MASK_LSISTOP, CKCU_POS_LSISTOP, Cmd);
}

/*********************************************************************************************************//**
 * @brief Enable or Disable the LSE stop function in the core power domain.
 * @param Cmd: This parameter can be ENABLE or DISABLE.
 * @retval None
 ************************************************************************************************************/
void CKCU_LSEStopCmd(ControlStatus Cmd)
{
  /* Check the parameters                                                                                   */
  Assert_Param(IS_CONTROL_STATUS(Cmd));

  CKCU_BF_WRITE(HT_CKCU->GCCR, CKCU_MASK_LSESTOP, CKCU_POS_LSESTOP, Cmd);
}
#endif
