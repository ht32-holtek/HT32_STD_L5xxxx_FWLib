/**
 @page RTC_Calendar_BackupDomain

 @verbatim
 * @file    RTC/Calendar_BackupDomain/readme.txt
 * @version V1.00
 * @date    2014-06-30
 * @brief   Description of RTC Calendar with Backup Domain example.
 @endverbatim
...
The RTC is in the VDD domain and powered by VDD when system reset,
the RTC configuration will not be lost if VDD is not switched off. 
...
1. After starting the program, check the backup data register value:
  - Register value is not correct(PWRCU_BAKREG_0 value will not be correct or has not yet
    ...
    Then store the initial time to PWRCU_BAKREG_1 and PWRCU_BAKREG_2.
  - Register value is correct: the RTC has been configured and the time will be displayed
    on HyperTerminal.
2. When an External Reset occurs, the VDD domain is not reset and the RTC configuration
   is not lost.
...
- RTC/Calendar_BackupDomain/main.c                Main program
- RTC/Calendar_BackupDomain/ht32lxxxxx_nn_it.c    Interrupt handlers
