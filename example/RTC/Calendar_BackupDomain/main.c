...
#define PWRCU_BAKREG_2_RTC_THIRD      (PWRCU_BAKREG_2)  /* Backup Register 2                                */
...
/*********************************************************************************************************//**
  ...
  *       - Enable RTC.
  *       - Write 0xAA55 to PWRCU_BAKREG_0 register.
...
int main(void)
{
  ...
  if (PWRCU_ReadBackupRegister((PWRCU_BAKREG_Enum) PWRCU_BAKREG_0_RTC_KEY) != 0xAA55)
  {
    ...
    PWRCU_WriteBackupRegister((PWRCU_BAKREG_Enum) PWRCU_BAKREG_0_RTC_KEY, 0xAA55);
  }
  else
  {
...
u8 AP_Time_Adjust(Time_T* AdjustTime)
{
  ...
  PWRCU_WriteBackupRegister((PWRCU_BAKREG_Enum) PWRCU_BAKREG_1_RTC_SECOND, ((secsum >> 16) & 0xFFFF));
  PWRCU_WriteBackupRegister((PWRCU_BAKREG_Enum) PWRCU_BAKREG_2_RTC_THIRD, (secsum & 0xFFFF));

  if (PWRCU_ReadBackupRegister((PWRCU_BAKREG_Enum) PWRCU_BAKREG_1_RTC_SECOND) != ((secsum >> 16) & 0xFFFF) || PWRCU_ReadBackupRegister((PWRCU_BAKREG_Enum) PWRCU_BAKREG_2_RTC_THIRD) != (secsum & 0xFFFF))
  {
    return 0;
  }
...
u8 AP_Time_Count(Time_T* CurrentTime)
{
  ...
  secsum = (PWRCU_ReadBackupRegister((PWRCU_BAKREG_Enum) PWRCU_BAKREG_1_RTC_SECOND) << 16) + PWRCU_ReadBackupRegister((PWRCU_BAKREG_Enum) PWRCU_BAKREG_2_RTC_THIRD) ;
