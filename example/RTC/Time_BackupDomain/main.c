...
int main(void)
{
  ...
  if (PWRCU_ReadBackupRegister(PWRCU_BAKREG_0) != 0xA55A)
  {
    ...
    PWRCU_WriteBackupRegister(PWRCU_BAKREG_1, ((wInitTime >> 16) & 0xFFFF));
    PWRCU_WriteBackupRegister(PWRCU_BAKREG_2, (wInitTime & 0xFFFF));
  ...
  while (1)
  {
    ...
    if (gwTimeDisplay == 1)
    {
      ...
      Time_Display(RTC_GetCounter() + (PWRCU_ReadBackupRegister(PWRCU_BAKREG_1) << 16) + PWRCU_ReadBackupRegister(PWRCU_BAKREG_2));
      gwTimeDisplay = 0;
    }
  }
}
