/*********************************************************************************************************//**
 * @file    USART/Singlewire_Master/main.c
 * @version $Rev:: 749          $
...
int main(void)
{
  ...
  while (1)
  {
    if(gRx_Match_Flag)
    {
      if(buffer_compare(gSlaveString ,gRx_Buffer ,gURSlave_Length))
      {
...
void UxART_Master_Tx(void)
{
  ...
  gURTx_Ptr = (u8 *)gMasterString;
  gURTx_Length = sizeof(gMasterString) - 1;
