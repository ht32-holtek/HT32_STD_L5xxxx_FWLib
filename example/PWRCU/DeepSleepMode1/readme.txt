/**
 @page PWRCU_DeepSleepMode1

 @verbatim
 * @file    PWRCU/DeepSleepMode1/readme.txt
 * @version V1.00
 * @date    2014-06-30
 * @brief   Description of PWRCU Deep Sleep Mode 1 example.
 @endverbatim

@par Example Description:

This example shows how to drive the system into Deep Sleep Mode 1 and be waken up by EXTI
Channel wakeup event and RTC compare match wakeup.

The KEY1 Button EXTI Channel is configured to generate wakeup event on low level.
...
- Configure EXTI channel to wake up the system on low level
- Turn off LED1
...
- LED1 off: system in Deep Sleep Mode 1
- LED2 is toggled if KEY1 EXTI Channel is used to exit from Deep Sleep Mode 1
- LED3 is toggled if RTC Compare Match is used to exit from Deep Sleep Mode 1

@par Directory Contents:

- PWRCU/DeepSleepMode1/main.c                   Main program
- PWRCU/DeepSleepMode1/ht32lxxxx_it.c           Interrupt handlers

@par Hardware and Software Environment:

- This example can be run on HT32 Series development board.

@note For power consumption measurement in Deep Sleep Mode 1:
